/** Pure JS SHA-256 (sync) for virtual file hashes — spec requires correctness vs VFS content */

function rotr(n: number, x: number): number {
  return (x >>> n) | (x << (32 - n))
}

/** Σ0 — round function */
function Sigma0(x: number): number {
  return rotr(2, x) ^ rotr(13, x) ^ rotr(22, x)
}

/** Σ1 — round function */
function Sigma1(x: number): number {
  return rotr(6, x) ^ rotr(11, x) ^ rotr(25, x)
}

/** σ0 — message schedule */
function sigma0(x: number): number {
  return rotr(7, x) ^ rotr(18, x) ^ (x >>> 3)
}

/** σ1 — message schedule */
function sigma1(x: number): number {
  return rotr(17, x) ^ rotr(19, x) ^ (x >>> 10)
}

const K = [
  0x428a2f98, 0x71374491, 0xb5c0fbcf, 0xe9b5dba5, 0x3956c25b, 0x59f111f1, 0x923f82a4, 0xab1c5ed5,
  0xd807aa98, 0x12835b01, 0x243185be, 0x550c7dc3, 0x72be5d74, 0x80deb1fe, 0x9bdc06a7, 0xc19bf174,
  0xe49b69c1, 0xefbe4786, 0x0fc19dc6, 0x240ca1cc, 0x2de92c6f, 0x4a7484aa, 0x5cb0a9dc, 0x76f988da,
  0x983e5152, 0xa831c66d, 0xb00327c8, 0xbf597fc7, 0xc6e00bf3, 0xd5a79147, 0x06ca6351, 0x14292967,
  0x27b70a85, 0x2e1b2138, 0x4d2c6dfc, 0x53380d13, 0x650a7354, 0x766a0abb, 0x81c2c92e, 0x92722c85,
  0xa2bfe8a1, 0xa81a664b, 0xc24b8b70, 0xc76c51a3, 0xd192e819, 0xd6990624, 0xf40e3585, 0x106aa070,
  0x19a4c116, 0x1e376c08, 0x2748774c, 0x34b0bcb5, 0x391c0cb3, 0x4ed8aa4a, 0x5b9cca4f, 0x682e6ff3,
  0x748f82ee, 0x78a5636f, 0x84c87814, 0x8cc70208, 0x90befffa, 0xa4506ceb, 0xbef9a3f7, 0xc67178f2,
]

export function sha256Bytes(message: Uint8Array): Uint8Array {
  const l = message.length * 8
  const withOne = new Uint8Array(((message.length + 9 + 63) >> 6) << 6)
  withOne.set(message)
  withOne[message.length] = 0x80
  new DataView(withOne.buffer).setUint32(withOne.length - 8, Math.floor(l / 0x100000000), false)
  new DataView(withOne.buffer).setUint32(withOne.length - 4, l >>> 0, false)

  let h0 = 0x6a09e667
  let h1 = 0xbb67ae85
  let h2 = 0x3c6ef372
  let h3 = 0xa54ff53a
  let h4 = 0x510e527f
  let h5 = 0x9b05688c
  let h6 = 0x1f83d9ab
  let h7 = 0x5be0cd19

  const w = new Uint32Array(64)
  for (let i = 0; i < withOne.length; i += 64) {
    for (let t = 0; t < 16; t++) {
      w[t] =
        (withOne[i + t * 4]! << 24) |
        (withOne[i + t * 4 + 1]! << 16) |
        (withOne[i + t * 4 + 2]! << 8) |
        withOne[i + t * 4 + 3]!
    }
    for (let t = 16; t < 64; t++) {
      w[t] = (sigma1(w[t - 2]!) + w[t - 7]! + sigma0(w[t - 15]!) + w[t - 16]!) >>> 0
    }

    let a = h0
    let b = h1
    let c = h2
    let d = h3
    let e = h4
    let f = h5
    let g = h6
    let h = h7
    for (let t = 0; t < 64; t++) {
      const t1 = (h + Sigma1(e) + ((e & f) ^ (~e & g)) + K[t]! + w[t]!) >>> 0
      const t2 = (Sigma0(a) + ((a & b) ^ (a & c) ^ (b & c))) >>> 0
      h = g
      g = f
      f = e
      e = (d + t1) >>> 0
      d = c
      c = b
      b = a
      a = (t1 + t2) >>> 0
    }
    h0 = (h0 + a) >>> 0
    h1 = (h1 + b) >>> 0
    h2 = (h2 + c) >>> 0
    h3 = (h3 + d) >>> 0
    h4 = (h4 + e) >>> 0
    h5 = (h5 + f) >>> 0
    h6 = (h6 + g) >>> 0
    h7 = (h7 + h) >>> 0
  }

  const out = new Uint8Array(32)
  const view = new DataView(out.buffer)
  view.setUint32(0, h0, false)
  view.setUint32(4, h1, false)
  view.setUint32(8, h2, false)
  view.setUint32(12, h3, false)
  view.setUint32(16, h4, false)
  view.setUint32(20, h5, false)
  view.setUint32(24, h6, false)
  view.setUint32(28, h7, false)
  return out
}

export function sha256Hex(input: string): string {
  const enc = new TextEncoder().encode(input)
  const hash = sha256Bytes(enc)
  return [...hash].map((b) => b.toString(16).padStart(2, '0')).join('')
}

export function sha256HexBytes(bytes: Uint8Array): string {
  const hash = sha256Bytes(bytes)
  return [...hash].map((b) => b.toString(16).padStart(2, '0')).join('')
}

export function verificationHash(parts: string[]): string {
  return sha256Hex(parts.join('|'))
}
