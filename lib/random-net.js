...
export function privateIpNear(baseIp, maskBits = 24, prng) {
  if (!baseIp) return privateIp(prng);
  const parts = baseIp.split('.').map(n=>parseInt(n,10));
  if (parts.length !== 4 || parts.some(isNaN)) return privateIp(prng);
  const sameA = parts[0], sameB = parts[1], sameC = (maskBits <= 16) ? parts[2] : parts[2];
  const a = sameA;
  const b = (maskBits <= 8) ? parts[1] : sameB;
  const c = (maskBits <= 16) ? parts[2] : sameC;
...
export function resolverIp(prng, internalBase) {
  // Sometimes internal gateway/resolver, sometimes public DNS like 8.8.8.8
  const choice = randInt(0, 1, prng);
  if (choice === 0 && internalBase) {
    const parts = internalBase.split('.');
    if (parts.length === 4) return `${parts[0]}.${parts[1]}.${parts[2]}.1`;
  }
  return pick(['8.8.8.8','1.1.1.1','9.9.9.9'], prng);
}

export function ephemeralPort(prng) {
  // IANA ephemeral range default 49152–65535
  return randInt(49152, 65535, prng);
}
...
export function makeSeed() {
  ...
  let seed = (a[0] ^ a[1] ^ (perf >>> 0) ^ (time >>> 0) ^ jitter) >>> 0;
