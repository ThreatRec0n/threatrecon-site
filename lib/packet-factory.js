...
import { uniqueId, randInt } from './random-net.js';
...
export function buildHTTP({ no, timeEpochMs, srcIp, dstIp, srcPort, dstPort = 80, method = 'GET', host, uri = '/', headers = {}, body = '' }) {
  ...
  const layers = {
    ...
    ip: { src: srcIp, dst: dstIp, srcIp, dstIp, version: 4, ttl: 64, protocolName: 'TCP', protocol: 6 },
...
export function buildDNS({ no, timeEpochMs, srcIp, dstIp, srcPort, dstPort = 53, qname, qtype = 'A' }) {
  const layers = {
    eth: { src: '00:00:00:00:00:00', dst: 'ff:ff:ff:ff:ff:ff' },
    ip: { src: srcIp, dst: dstIp, srcIp, dstIp, version: 4, ttl: 128, protocolName: 'UDP', protocol: 17 },
    udp: { srcPort, dstPort },
    dns: { qname, query: qname, qtype, type: qtype, id: randInt(1, 65535) },
  };
...
export function buildICMP({ no, timeEpochMs, srcIp, dstIp, type = 'Echo (ping) request', code = 0 }) {
  const layers = {
    eth: { src: '00:00:00:00:00:00', dst: 'ff:ff:ff:ff:ff:ff' },
    ip: { src: srcIp, dst: dstIp, srcIp, dstIp, version: 4, ttl: 128, protocolName: 'ICMP', protocol: 1 },
    icmp: { type, code },
  };
...
export function buildRTP({ no, timeEpochMs, srcIp, dstIp, srcPort, dstPort, ssrc, seq, pt = 0 }) {
  const layers = {
    eth: { src: '00:00:00:00:00:00', dst: 'ff:ff:ff:ff:ff:ff' },
    ip: { src: srcIp, dst: dstIp, srcIp, dstIp, version: 4, ttl: 64, protocolName: 'UDP', protocol: 17 },
    udp: { srcPort, dstPort },
    rtp: { ssrc, seq, pt },
  };
...
export function buildSIP({ no, timeEpochMs, srcIp, dstIp, srcPort = 5060, dstPort = 5060, method = 'INVITE', callId, from = 'sip:alice@local', to = 'sip:bob@local', sdp = '' }) {
  const start = `${method} sip:${to} SIP/2.0\r\n`;
  const headers = [
    `Via: SIP/2.0/UDP ${srcIp}:${srcPort}`,
    `From: <${from}>`,
    `To: <sip:${to}>`,
    `Call-ID: ${callId}`,
    `CSeq: 1 ${method}`,
    `Contact: <${from}>`,
    `Content-Type: ${sdp ? 'application/sdp' : 'text/plain'}`,
    `Content-Length: ${sdp.length}`,
  ].join('\r\n');
  const wire = `${start}${headers}\r\n\r\n${sdp}`;
  const layers = {
    eth: { src: '00:00:00:00:00:00', dst: 'ff:ff:ff:ff:ff:ff' },
    ip: { src: srcIp, dst: dstIp, srcIp, dstIp, version: 4, ttl: 64, protocolName: 'UDP', protocol: 17 },
    udp: { srcPort, dstPort },
    sip: { method, callId, from, to, hasSDP: !!sdp },
  };
  return assembleBase({ no, timeEpochMs, src: `${srcIp}:${srcPort}`, dst: `${dstIp}:${dstPort}`, protocol: 'SIP', info: `${method} ${to}`, length: wire.length + 66, layers, payloadAscii: wire });
}

export function buildSMB({ no, timeEpochMs, srcIp, dstIp, srcPort, dstPort = 445, op = 'Write', share = '\\share', path = '\\path\\file' }) {
  const layers = {
    eth: { src: '00:00:00:00:00:00', dst: 'ff:ff:ff:ff:ff:ff' },
    ip: { src: srcIp, dst: dstIp, srcIp, dstIp, version: 4, ttl: 128, protocolName: 'TCP', protocol: 6 },
    tcp: { srcPort, dstPort, flags: 'ACK', seq: randInt(1, 1e6), ack: randInt(1, 1e6) },
    smb: { share, path, op },
  };
