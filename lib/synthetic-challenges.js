...
const dataExfiltrationScenario = {
  ...
  generator: (config) => {
    ...
    packets.push({
      id: evidenceId,
      ts: evidenceTs,
      tsISO: new Date(evidenceTs).toISOString(),
...
const credentialLeakScenario = {
  ...
  generator: (config) => {
    ...
    for (let i = 0; i < config.count - 3; i++) {
      packets.push({
        id: `pkt-${i}`,
        ts: new Date(baseTime + i * 5000).toISOString(),
    ...
    packets.push({
      id: evidenceId,
      ts: new Date(baseTime + config.count * 5000).toISOString(),
      src: `${srcIp}:${randomPort()}`,
      dst: `${targetIp}:80`,
      proto: 'HTTP',
      length: evidenceBytes.length + 100,
...
const dnsTunnelingScenario = {
  ...
  generator: (config) => {
    ...
    for (let i = 0; i < config.count - 5; i++) {
      packets.push({
        id: `pkt-${i}`,
        ts: new Date(baseTime + i * 3000).toISOString(),
        src: `${srcIp}:${randomPort()}`,
        dst: `${randomIP()}:80`,
        proto: 'HTTP',
    ...
    packets.push({
      id: evidenceId,
      ts: new Date(baseTime + config.count * 3000).toISOString(),
      src: `${srcIp}:${randomPort()}`,
      dst: `${dnsServer}:53`,
      proto: 'DNS',
...
const audioExfiltrationScenario = {
  ...
  generator: (config) => {
    ...
    for (let i = 0; i < config.count - 1; i++) {
      packets.push({
        id: `pkt-${i}`,
        ts: new Date(baseTime + i * 20000).toISOString(),
        src: `${srcIp}:${randomPort()}`,
        dst: `${randomIP()}:80`,
        proto: 'UDP',
    ...
    packets.push({
      id: evidenceId,
      ts: new Date(baseTime + config.count * 20000).toISOString(),
      src: `${srcIp}:16384`,
      dst: `${targetIp}:16384`,
      proto: 'RTP',
      length: 160,
...
const commandAndControlScenario = {
  ...
  generator: (config) => {
    ...
    for (let i = 0; i < config.count; i++) {
      ...
      packets.push({
        id: `pkt-${i}`,
        ts: new Date(baseTime + i * 6000).toISOString(),
