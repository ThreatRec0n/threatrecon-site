import { safeDate } from './safe-time.js';

export const PACKETS = { beginner:25, intermediate:50, advanced:100 };
export const EVIDENCE_RANGE = { beginner:[1,2], intermediate:[2,4], advanced:[3,7] };

export function packetBudgetForDifficulty(diff) {
  const d = String(diff || '').toLowerCase();
  const total = d in PACKETS ? PACKETS[d] : 25;
  const [evMin, evMax] = d in EVIDENCE_RANGE ? EVIDENCE_RANGE[d] : [1,2];
  return { total, evMin, evMax };
}
...
import { makeSeed } from './random-net.js';
import { SCENARIOS, pickPacketCount, pickEvidenceCount } from './scenario-catalog.js';
