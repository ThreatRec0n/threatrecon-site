# 🎯 COMPLETE FIX PROMPT FOR CURSOR
...
- Make sure OnboardingTutorial component is imported and rendered in app/simulation/page.tsx
...
- Ensure "🎓 Replay Tutorial" button clears this flag and re-shows tutorial
...
- Style it with: `text-sm text-blue-400 hover:text-blue-300 cursor-help`
- Icon: Use HelpCircle from lucide-react
...
<div className="flex items-center gap-2">
  ...
  {alert.technique_id && (
    <button
      onClick={(e) => {
        e.stopPropagation();
        openTechniqueExplainer(alert.technique_id);
      }}
...
- Shows content from lib/learning/mitre-knowledge.ts for the given technique ID
- Sections to display:
  - 🎯 Why This is Suspicious (red flags list)
  - 🔍 How to Investigate (step-by-step list)
  - 📖 Real-World Example (story)
  - 🛡️ How to Defend (recommendations)
  - 🔗 Related Techniques (links to other techniques)
...
- Pass openTechniqueExplainer function to AlertQueue component as prop
- Render TechniqueExplainerPanel at bottom of page

#### 4. Add Learning Mode Toggle (Optional but Nice):
...
Create a component with 4 difficulty cards:
- 🌱 Beginner: 100 events, 3 alerts, 5-8 IOCs, 2x SLA time
- 📊 Intermediate: 500 events, 6 alerts, 15-20 IOCs, normal SLA
- 🎯 Advanced: 2000 events, 10 alerts, 30-40 IOCs, 0.8x SLA
- 💀 Expert: 5000 events, 20 alerts, 60-80 IOCs, 0.5x SLA

Each card shows:
- Icon, name, description
- Stats (events, alerts, IOCs, SLA time)
...
- Add difficulty parameter to event generation functions
- Adjust event counts based on difficulty:
  - Beginner: 100 total events
  - Intermediate: 500 total events
  - Advanced: 2000 total events
  - Expert: 5000 total events

**In lib/simulation-engine/alert-factory.ts:**
- Adjust alert counts based on difficulty:
...
- Make it clickable to change difficulty
...
- Add `briefing` field: string (short description)
...
- Include [Start Investigation] button
...
☐ 1. Add all missing data-tutorial attributes (5 locations)
☐ 2. Auto-trigger tutorial on first visit
☐ 3. Add "?" help icons to all alert cards
☐ 4. Create/update TechniqueExplainerPanel component
☐ 5. Integrate TechniqueExplainerPanel into simulation page
☐ 6. Create DifficultySelector component
☐ 7. Integrate difficulty selection before starting investigation
☐ 8. Adjust event/alert generation based on selected difficulty
☐ 9. Add difficulty badge to header
☐ 10. Add APT group badges to alerts
☐ 11. Add XP progress bar to header
☐ 12. Add achievement unlock toasts
